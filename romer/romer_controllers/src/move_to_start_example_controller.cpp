...
#include <romer_controllers/move_to_start_example_controller.hpp>
...
controller_interface::return_type MoveToStartExampleController::update(
  ...
  if (fin_count_ > 0 || true) {
    const double kAlpha = 0.99;
    dq_filtered_ = (1 - kAlpha) * dq_filtered_ + kAlpha * dq_;
    Vector7d tau_d_calculated =
        k_gains_.cwiseProduct(q_desired - q_) + d_gains_.cwiseProduct(-dq_filtered_);
    if (fabs((q_goal_ - q_).maxCoeff()) < 1.2e-2) fin_count_--;
    for (int i = 0; i < 7; ++i) {
      // if (i == 5) RCLCPP_INFO(get_node()->get_logger(), "Interface %d set to %f", i, error);
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(romer_controllers::MoveToStartExampleController,
